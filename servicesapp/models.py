...
class ServiceApplication(models.Model):
    ...
    charges = models.DecimalField(max_digits=5, decimal_places=2, blank=True, null=True)
    updated = models.DateTimeField(auto_now=True)
    created = models.DateTimeField(auto_now_add=True)
    userId  = models.ForeignKey(User, on_delete=models.PROTECT, related_name="student")
    staffInCharged  = models.ForeignKey(User, on_delete=models.PROTECT, related_name="staff", blank=True, null=True)
    equipmentId  = models.ForeignKey(Equipment, on_delete=models.PROTECT, related_name="equipment", null=True, blank=True)
